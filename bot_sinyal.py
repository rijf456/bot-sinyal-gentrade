...
/markwin [id] - /markloss [id] - /setmodal [jumlah] - Atur modal (USDT)
"""
...
def detect(df):
    ...
    if last['close'] > last['ema12'] and last['close'] > last['ema26']:
        signal = 'bull'; reasons.append('MA Breakout')
    elif last['close'] < last['ema12'] and last['close'] < last['ema26']:
        signal = 'bear'; reasons.append('MA Breakdown')
...
{size_str}
""".strip()
            send_message(f"🚀 *Sinyal Baru Terdeteksi* untuk `{sym}`!")
...
/history [pair] - Riwayat sinyal per pair
/detail [id] - Detail satu sinyal
/markwin [id] - /markloss [id] - /setmodal [jumlah] - Atur modal untuk rekomendasi size
...
"""
            ...
            args = text.split(maxsplit=1)
            if len(args) < 2:
                send_message("Format: `/markwin [id]` atau `/markloss [id]`")
            else:
